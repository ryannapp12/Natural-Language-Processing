...
def train_model(model, loss_fn, optimizer, train_generator, dev_generator):
    ...
    """
    ########## YOUR CODE HERE ##########
    # Given a model, data, and loss function, you should do the following:
    ...
    # Make sure to print the dev set loss each epoch to stdout.
    for epoch in range(2):
        ...
        with torch.no_grad():
            for X_b, Y_b in dev_generator:
                ...
                loss += loss_fn(predictions, Y_b).data
...
def main(args):
    ...
    ########## YOUR CODE HERE ##########
    # for each of the two models, you should
    # 1) create it,
    # 2) run train_model() to train it
    # 3) run test_model() on the result
    if args.model == "dense":
        model = models.DenseNetwork(embeddings)
    elif args.model == "RNN":
        model = models.RecurrentNetwork()
    elif args.model == "extensions1":
        pass
